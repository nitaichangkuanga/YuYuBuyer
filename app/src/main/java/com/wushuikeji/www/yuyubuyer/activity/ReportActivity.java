...
import com.wushuikeji.www.yuyubuyer.adapter.ReportListViewAdapter;
...
import com.wushuikeji.www.yuyubuyer.jsonparse.ReportListParse;
...
public class ReportActivity extends AppCompatActivity {
    ...
    //进度条
    @InjectView(R.id.pb_reportLoading_progressBar)
    ProgressBar pb_reportLoading_progressBar;

    //空视图
    @InjectView(R.id.tv_report_emptyView)
    TextView tv_report_emptyView;

    private ReportListViewAdapter mReportListViewAdapter;

    private List<ReportBean> reportBeanList = new ArrayList<ReportBean>();

    //举报列表的地址
    private String reportListUrl = Constants.commontUrl + "appeal/report_list";
    ...
    private Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case LOADING:
                    pb_reportLoading_progressBar.setVisibility(View.VISIBLE);
                    tv_report_emptyView.setVisibility(View.GONE);
                    in_returnApply_pf.setVisibility(View.GONE);
                    break;
                case SUCCESS:
                    if (reportBeanList.size() != 0) {
                        pb_reportLoading_progressBar.setVisibility(View.GONE);
                        tv_report_emptyView.setVisibility(View.GONE);
                        in_returnApply_pf.setVisibility(View.VISIBLE);
                        //更新数据
                        if (mReportListViewAdapter != null) {
                            mReportListViewAdapter.notifyDataSetChanged();
                        }
                    } else {
                        pb_reportLoading_progressBar.setVisibility(View.GONE);
                        tv_report_emptyView.setVisibility(View.VISIBLE);
                        in_returnApply_pf.setVisibility(View.GONE);
                    }
                    break;
                default:
    ...
    private void initView() {
        ...
        mReportListViewAdapter = new ReportListViewAdapter(reportBeanList);

        returnApply_listView.setAdapter(mReportListViewAdapter);
        //刷新的操作
    ...
    private void initEvent() {
        ...
        in_returnApply_pf.setPtrHandler(new PtrHandler() {
            ...
            public void onRefreshBegin(PtrFrameLayout frame) {
                if (!ISNetworkConnectUtils.iSNetworkConnect(UIUtils.getContext())) {
                    ...
                    if (reportBeanList.size() <= 8 && returnApply_listView.getFooterViewsCount() == 1) {
                        try {
        ...
        returnApply_listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ToNextActivityUtils.toNextActivityNotFinishAndParameters(ReportActivity.this, ReportDetailsActivity.class, "reportId", reportBeanList.get(position).reportId);
            }
        });
    }
    ...
    private void loadNetworkData(String offset, boolean isNeedLoading, boolean isFromRefresh, boolean isNeedClearData) {
        ...
        OkHttpUtils.post().url(reportListUrl).addParams("user_id", mUserId)
            ...
            public void onResponse(String response, int id) {
                try {
                    ...
                    if ("0".equals(response_code)) {
                        //可以下一步解析
                        List<ReportBean> tempList = ReportListParse.reportListParse(response);

                        if (tempList != null) {
                            if (commonItemIsNeedClearData) {
                                ReportActivity.this.reportBeanList.clear();
                                ReportActivity.this.reportBeanList.addAll(tempList);
                                ListViewUtils.isAddListViewReportFooterView(reportBeanList, returnApply_listView, mFooterView);
                            } else {
                                //加载更多
                                if (tempList.size() == 0) {
                                    ...
                                    reportBeanList.addAll(tempList);
                                    ListViewUtils.isAddListViewReportFooterView(reportBeanList, returnApply_listView, mFooterView);
                                }
                            }
                            //刷新完成
