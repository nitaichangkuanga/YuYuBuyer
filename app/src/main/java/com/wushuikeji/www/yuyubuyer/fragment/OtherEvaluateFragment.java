...
public class OtherEvaluateFragment extends BaseFragment {
    ...
    TextView tv_otherEvaluateFragment_emptyView;

    //刷新的类
    @InjectView(R.id.in_shopEvaluateFragment_pf)
    PtrClassicFrameLayout mOtherEvaluateFragmentPtrFrame;

    @InjectView(R.id.shopEvaluateFragment_listView)
    ListView otherEvaluateFragment_listView;

    //进度条
    @InjectView(R.id.pb_shopEvaluateFragment_progressBar)
    CircularProgress pb_otherEvaluateFragment_progressBar;
    ...
    private Handler mHandler = new Handler(){
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case LOADING:
                    pb_otherEvaluateFragment_progressBar.setVisibility(View.VISIBLE);
                    tv_otherEvaluateFragment_emptyView.setVisibility(View.GONE);
                    mOtherEvaluateFragmentPtrFrame.setVisibility(View.GONE);
                    break;
                case SUCCESS:
                    //ListView的初始化相关操作
                    operateListView();
                    ...
                    if(allEvaluateBeanList.size() != 0) {
                        pb_otherEvaluateFragment_progressBar.setVisibility(View.GONE);
                        tv_otherEvaluateFragment_emptyView.setVisibility(View.GONE);
                        mOtherEvaluateFragmentPtrFrame.setVisibility(View.VISIBLE);
                        if(mAllEvaluatewListViewAdapter != null) {
                            mAllEvaluatewListViewAdapter.notifyDataSetChanged();
                        }
                    }else {
                        pb_otherEvaluateFragment_progressBar.setVisibility(View.GONE);
                        tv_otherEvaluateFragment_emptyView.setVisibility(View.VISIBLE);
                        mOtherEvaluateFragmentPtrFrame.setVisibility(View.GONE);
                    }
                    break;
                default:
    ...
    public View initView() {
        ...
        //刷新的操作
        if(mOtherEvaluateFragmentPtrFrame != null) {
            mOtherEvaluateFragmentPtrFrame.setLastUpdateTimeRelateObject(this);
            PtrFrameRefreshUtils.setRefreshParams(mOtherEvaluateFragmentPtrFrame);
        }
    ...
    public void initEvent() {
        super.initEvent();
        //空视图的点击事件
        tv_otherEvaluateFragment_emptyView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                pb_otherEvaluateFragment_progressBar.setVisibility(View.GONE);
                tv_otherEvaluateFragment_emptyView.setVisibility(View.GONE);
                mOtherEvaluateFragmentPtrFrame.setVisibility(View.VISIBLE);
                mOtherEvaluateFragmentPtrFrame.autoRefresh();
            }
        });

        //listView的滑动事件
        otherEvaluateFragment_listView.setOnScrollListener(new AbsListView.OnScrollListener() {
        ...
        //实现下拉刷新
        mOtherEvaluateFragmentPtrFrame.setPtrHandler(new PtrHandler() {
            @Override
            public boolean checkCanDoRefresh(PtrFrameLayout frame, View content, View header) {

                // here check $mListView instead of $content
                return PtrDefaultHandler.checkContentCanBePulledDown(frame, otherEvaluateFragment_listView, header);
            }
    ...
    private void updateData() {
        ...
        //添加数据到数据源
        moniData();
        ListViewUtils.isAddListViewAllEvaluateFooterView(allEvaluateBeanList,otherEvaluateFragment_listView,mFooterView);
        //刷新完成
        mOtherEvaluateFragmentPtrFrame.refreshComplete();
        if(mAllEvaluatewListViewAdapter != null) {
            mAllEvaluatewListViewAdapter.notifyDataSetChanged();
        }
    }
    ...
    private void operateListView() {
        ...
        otherEvaluateFragment_listView.setAdapter(mAllEvaluatewListViewAdapter);
    }
}
