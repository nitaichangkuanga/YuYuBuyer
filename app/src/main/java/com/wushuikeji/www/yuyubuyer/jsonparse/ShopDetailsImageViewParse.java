 ...
 * @des 解析图片
 */
public class ShopDetailsImageViewParse {
    ...
    public static List<ImageViewBean> imageViewParse(String jsonString) {
        ...
        try {
            ...
            JSONArray shop_imagesArray = obj.getJSONArray("shop_images");
            if (shop_imagesArray == null) {
                return null;
            } else {
                for (int i = 0; i < shop_imagesArray.length(); i++) {
                    ImageViewBean imageViewBean = new ImageViewBean();
                    JSONObject item_obj = shop_imagesArray.getJSONObject(i);
                    //图片的ID
